...
{if $image_content.is_valid}
    ...
    {if $link_to_image}
        {set href = cond($fullpath, $image_content['original'].url|ezroot(,'full'), $image_content['original'].url|ezroot)}
    {/if}
    ...
    {if and( is_set( $image ), $image )}
        ...
        {/if}
        {if $href}<a href={$href}{if and( is_set( $link_class ), $link_class )} class="{$link_class}"{/if}{if and( is_set( $link_id ), $link_id )} id="{$link_id}"{/if}{if $target} target="{$target}"{/if}{if and(is_set($onclickaction), $onclickaction)} onClick="{$onclickaction}"{/if}{if is_set($rel)} rel="{$rel}"{/if}>{/if}
        <img src={cond($fullpath, concat('"http://www.nuxeo.com',$image.url|ezroot('no'),'"'), $image.url|ezroot)} width="{$image.width}" height="{$image.height}" {if $hspace}hspace="{$hspace}"{/if} style="{$inline_style}" alt="{$alt_text|wash(xhtml)}" title="{$title|wash(xhtml)}" />
        {if $href}{if is_set($magnify)}<span class="magnify"{if is_set( $magnify_title )} title="{$magnify_title}"{/if}></span>{/if}</a>{/if}
    {/if}
