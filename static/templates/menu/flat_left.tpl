...
{if ne( $pagedata.class_identifier, 'documentation_page' )}
    {def $root_node=fetch( 'content', 'node', hash( 'node_id', $pagedata.path_array[$left_menu_depth].node_id ) )
         $left_menu_items = fetch( 'content', 'list', hash( 'parent_node_id', $root_node.node_id,
                                                            ...
                                                            'extended_attribute_filter', hash(
                                                              'id','InMenuFilter',
                                                              'params', hash('menu', 'left', 'classes', array('folder','product_sheet','solution','resource_center','resource_category','plain_page','search_engine_partner'))
                                                            ) ) )
    ...
    {if $left_menu_items_count}
        ...
        {foreach $left_menu_items as $key => $item}
            ...
            {else}
            	{set $li_class = $li_class|append("notlastli")}
            {/if}
            ...
            {if and( eq( $current_node_in_path_2, $item.node_id ), ne( $item.node_id, 104 ) )}
                {def $sub_menu_items = fetch( 'content', 'list', hash( 'parent_node_id', $item.node_id,
                                                                      'sort_by', $item.sort_array,
                                                                      'data_map_load', false(),
                                                                      'extended_attribute_filter', hash(
                                                                          'id','InMenuFilter',
                                                                          'params', hash('menu', 'left', 'classes', array('folder','product_sheet','solution','resource_center','resource_category','plain_page'))
                                                                       ) ) )
                ...
                <ul class="submenu-list">
                    ...
                    {else}
                        <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="third_level_menu"><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $subitem.node_id)|ezurl}{else}{$subitem.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}>{$subitem.name|wash()}</a></div>
                    {/if}

                    {if and( eq( $current_node_in_path_3, $subitem.node_id ), ne( $subitem.node_id, 104 ) )}
                        ...
                        {if $sub_sub_menu_items_count}
                            <ul class="subsubmenu-list">
                                {foreach $sub_sub_menu_items as $sub_subkey => $sub_subitem}
                                   {set $a_class = cond($current_node_in_path_4|eq($sub_subitem.node_id), array("selected"), array())
                                     $li_class = cond( $sub_subkey|eq(0), array("firstli"), array() )}
                                {if $sub_sub_menu_items_count|eq( $sub_subkey|inc )}
                                    {set $li_class = $li_class|append("lastli")}
                                {/if}
                                {if $sub_subitem.node_id|eq( $current_node_id )}
                                    {set $a_class = $a_class|append("current")}
                                {/if}
                                {if eq( $sub_subitem.class_identifier, 'link')}
                                    <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="fourth_level_menu"><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $sub_subitem.node_id)|ezurl}{else}href={$sub_subitem.data_map.location.content|ezurl} target="_blank"{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if} title="{$sub_subitem.data_map.location.data_text|wash}">{$sub_subitem.name|wash()}</a></div></li>
                                {else}
                                    <li{if $li_class} class="{$li_class|implode(" ")}"{/if}><div class="fourth_level_menu"><a href={if eq( $ui_context, 'browse' )}{concat("content/browse/", $sub_subitem.node_id)|ezurl}{else}{$sub_subitem.url_alias|ezurl}{/if}{if $a_class} class="{$a_class|implode(" ")}"{/if}>{$sub_subitem.name|wash()}</a></div></li>
                                {/if}
                                {/foreach}
                            </ul>
                        {/if}
